...
A75V25 relaxation tests, restricted to the loading ramp (same convention
as plot_stress_strain.py) -- see plot_stress_time.py for the rate-group
structure and the partial-dataset/rate-labeling caveats.
...
tracking artifacts rather than real material response. Stress vs.
position is included alongside for the same reason plot_stress_strain.py
exists: a reference view of the mechanically-measured signal (stress)
against another mechanically-measured signal (position), unaffected by
any video-tracking noise, to compare against.
