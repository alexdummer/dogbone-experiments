"""Concatenate the raw chunks of each A25V75 relaxation test (tests 1-6,
variable number of _N continuation chunks) and the two single-file cyclic
tests (7, 8), extract time, position, force, and strain, and zero
position/force/strain against the pre-ramp baseline (before position
starts moving) rather than the raw first sample."""
...
RELAX_PATTERN = re.compile(r"^(A25V75-(\d+)_\d{8}_\d{6})\.csv$")
CYCLIC_PATTERN = re.compile(r"^A25V75-(\d+)-cyclic_\d{8}_\d{6}\.csv$")


def find_relax_tests():
    tests = {}
    for path in RAW_DIR.glob("A25V75-*.csv"):
...
def find_cyclic_tests():
    tests = {}
    for path in RAW_DIR.glob("A25V75-*-cyclic_*.csv"):
        m = CYCLIC_PATTERN.match(path.name)
        if m:
            tests[int(m.group(1))] = path
    return dict(sorted(tests.items()))
...
def main():
    ...
    for test_id, (main_path, stem) in find_relax_tests().items():
        ...
        out_path = CLEANED_DIR / f"A25V75-{test_id}_cleaned.csv"
    ...
    for test_id, path in find_cyclic_tests().items():
        df = load_cyclic_test(path)
        df, onset = zero_to_baseline(df)
        out_path = CLEANED_DIR / f"A25V75-{test_id}_cyclic_cleaned.csv"
        df.to_csv(out_path, index=False)
        print(
