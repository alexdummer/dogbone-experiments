"""Stress-vs-strain plot for the A100V0 relaxation tests, all loading rates
...
structure and the absolute-rate caveat). Strain here is the two-point
videoextensometer measurement (A100V0-relax/sync_and_clean_data.py), valid
only over the ramp -- exactly the region this plot uses. Legend entries
report the actual measured mean engineering strain rate over the ramp
(not the nominal crosshead displacement rate) -- see
...
def main():
    ...
    for label in RATE_LABELS:
        ...
        ax.plot(strain, stress, color=color, linewidth=1.8, linestyle="--",
                label=rf"{label} rate ($\dot\varepsilon\approx${100 * rate:.3g}\%/s)")

    ax.set_xlabel("Engineering strain in \\%")
    ax.set_ylabel("Engineering stress in MPa")
